...
class PostsManager {
    ...
    async init() {
        ...
        const posts = this.getAllPostsSync();
        if (!posts.length) {
    ...
    async loadFromJSON() {
        try {
            const response = await fetch(this.jsonFile + '?t=' + Date.now()); // Cache busting
            if (response.ok) {
    ...
    // Get all posts synchronously (for internal use)
    getAllPostsSync() {
    ...
    // Get all posts (async version - waits for init)
    async getAllPosts() {
        await this.initPromise;
        return this.getAllPostsSync();
    }
    
    // Get all posts (sync version for backward compatibility)
